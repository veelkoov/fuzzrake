...
export function requireJQ(selector: string, min: number = 1, max: number|null = 1): JQuery<HTMLElement> {
    const result = jQuery(selector);

    if (result.length < min || (max !== null && result.length > max)) {
        DarnIt.report('Failed matching HTML elements. Most probably this error happened because the maintainer introduced some changes on the website in a way which you would expect from an unskilled person.', `Looked for: ${selector}, count: ${min}..${max}, matched: ${result.length}`, true);
    }
