<template>
  ...
  >
    <div v-if="aasConfig.getMakerMode()" class="card border-danger mb-3">
      ...
      <div class="card-body">
        <p class="card-text">
          All filtering has been temporarily disabled to ease searching the
          whole database. Use the below button to restore them.
        </p>
        <a
          id="btn-reenable-filters"
          :href="Static.getMainPath()"
          class="btn btn-light btn-outline-danger"
          @click="disableMakerMode"
          >Re-enable filters</a
        >
      </div>
    </div>
        ...
        <div class="col-md-6">
          ...
          >
            ...
            >
              Filters
              <span
                v-if="state.activeFiltersCount"
                class="badge rounded-pill text-bg-light"
                >{{ state.activeFiltersCount }}</span
              >
            </button>
            <button
              type="button"
    ...
    if (window.location.hash.match(makerIdHashRegexp)) {
      ...
      if (!this.aasConfig.getMakerMode()) {
        this.messageBus.requestDataLoad(
          "wantsSfw=0&isAdult=1&inactive[]=.&makerId=" +
