...
export default class Filter {
  ...
  public saveChoices(): void {
    Storage.saveString(
      `filters/${this.groupName}/choices`,
      Array.from(this.state.valuesToLabels.keys()).join("\n"),
    );
  }
  ...
  private getValidValueLabelPairsFromOptions(
    options: FilterOptions,
  ): Map<string, string> {
    const result = new Map<string, string>();

    options.specialItems.forEach((option) =>
      result.set(option.value, option.label),
    );

    this.getValidValueLabelPairsFromItems(options.items).forEach((value, key) =>
  ...
  private getValidValueLabelPairsFromItems(
    options: Items,
  ): Map<string, string> {
    const result = new Map<string, string>();
    ...
    options.forEach((option) => {
      ...
      this.getValidValueLabelPairsFromItems(option.subitems).forEach(
