<template>
    ...
    <table id="artisans" class="table table-striped table-sm table-hover">
      <thead class="table-dark">
        <tr>
          ...
          <th
            v-for="(label, name, index) of columns.columns"
            v-show="columns.isVisible(name)"
            :key="name"
            :class="[
              name,
              index === columns.count() - 1 ? 'text-end' : 'text-center',
            ]"
          >
            {{ label }}
          </th>
      ...
      <tbody>
        <template
          v-for="(artisan, index) of artisans"
          :key="artisan.lastMakerId"
        >
          <tr
          ...
          >
            ...
            <td
              v-show="columns.isVisible('makerId')"
              class="makerId"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td
              v-show="columns.isVisible('state')"
              class="state"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td
              v-show="columns.isVisible('languages')"
              class="languages"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td
              v-show="columns.isVisible('productionModels')"
            ...
            <td
              v-show="columns.isVisible('styles')"
              class="styles"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td
              v-show="columns.isVisible('types')"
              class="types"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td
              v-show="columns.isVisible('features')"
              class="features"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td
              v-show="columns.isVisible('species')"
              class="species"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            >
              <ul v-if="artisan.gotSpeciesInfo">
                ...
                <li
                  v-for="item in artisan.speciesDoesnt"
                  :key="item"
                  class="no"
                >
            ...
            <td
              v-show="columns.isVisible('commissions')"
              class="commissions"
              data-bs-toggle="modal"
              data-bs-target="#artisanDetailsModal"
              @click="setSubject(artisan)"
            ...
            <td v-show="columns.isVisible('links')" class="links">
              <div
                class="btn-group artisan-links"
                role="group"
                aria-label="Links to websites"
              >
                <a
                  v-if="isDevEnv()"
                  class="btn btn-warning"
                  :href="Static.getArtisanEditPath(artisan.lastMakerId)"
                  ><i class="fas fa-edit"
                /></a>

                <a
                  v-if="artisan.fursuitReviewUrl"
                  class="btn btn-secondary"
                  :href="artisan.fursuitReviewUrl"
                  target="_blank"
                ...
                <a
                  class="btn btn-secondary"
                  :href="
                    'https://bewares.getfursu.it/#search:' +
                    artisan.abSearchJson
                  "
                  target="_blank"
                ...
                <div class="btn-group" role="group">
                  ...
                  <ul
                    class="dropdown-menu"
                    :aria-labelledby="'drpdwnmn' + index.toString()"
                  >
                    <TblLink
                      :url="
                        'https://bewares.getfursu.it/#search:' +
                        artisan.abSearchJson
                      "
                      icon-class="fa-solid fa-spell-check"
                      label="Check on Artists Beware"
                    />
                    <TblLink
                    ...
                    <li>
                      <a
                        ...
                        @click="setSubject(artisan)"
                      >
                        <i class="fas fa-exclamation-triangle" /> Data
                        outdated/inaccurate?
                      </a>
                    </li>
                  </ul>
  ...
  <p id="artisans-table-count" class="small">
    Displaying {{ artisans.length }} out of
    {{ Static.getTotalArtisansCount() }} fursuit makers in the database.
  </p>
</template>
...
export default class Table extends Vue {
  ...
  private matchedMakerId(artisan: Artisan): boolean {
    return (
      this.state.search.isMakerId &&
      artisan.hasMakerId(this.state.search.textUc)
    );
  }
  ...
  private handleCardOpeningOnMakerIdInUrlsHash() {
    ...
    if (1 !== this.artisans.length || !this.artisans[0].hasMakerId(makerId)) {
      console.log(
        `Failed opening card for ${this.state.openCardForMakerId}, loaded ${this.artisans.length} records`,
      );
    } else {
      // FIXME: https://github.com/veelkoov/fuzzrake/pull/187/files
      // eslint-disable-next-line no-undef
      nextTick(() =>
        jQuery("#artisans tbody tr:first-child td:first-child").trigger(
          "click",
        ),
      );
    }
