    ...
    <div class="modal-dialog modal-xl">
      <div id="artisanDetailsModalContent" class="modal-content artisan-data">
            ...
            <template v-if="artisan.makerId">
              <a id="makerId" :href="'#' + artisan.makerId">
                <span class="badge bg-secondary"
                  ><i class="fas fa-link" /> {{ artisan.makerId }}</span
                >
              </a>
              <wbr />
            </template>

            <span id="artisanName">{{ artisan.name }}</span
            >&nbsp;<span
              class="flag-icon"
              :class="'flag-icon-' + artisan.lcCountry"
            />

            <small>
              Based in <OptionalText :item="artisan.location" />; crafting since
        ...
        <div class="modal-body row px-4 py-2">
          <div
            v-if="artisan.inactiveReason"
            class="col-md-12 p-1 alert alert-warning"
            role="alert"
          >
            <strong
              >This maker has been hidden for the following reason:</strong
            >
            <br />
          ...
          <div
            v-if="hasPhotos()"
            class="col-md-12 p-1 overflow-auto imgs-container"
          >
            <div v-for="(item, index) in artisan.photoUrls" :key="item">
              <a :href="item" target="_blank"
                ><img :src="artisan.miniatureUrls[index]" alt=""
              /></a>
            </div>
          </div>
          ...
          <div class="col-md-12 p-1">
            <p class="mb-0 small">
              ...
              <AgesDescription :add-text="true" :artisan="artisan" />
            </p>
          </div>
          ...
          <div class="col-md-3 p-1">
            ...
            <div class="small pb-2">
              <OptionalList
                :comment="artisan.productionModelsComment"
                :items="artisan.productionModels"
                :other-items="[]"
              />
            </div>
            ...
            <div class="small">
              <OptionalList
                :comment="artisan.stylesComment"
                :items="artisan.styles"
                :other-items="artisan.otherStyles"
              />
            </div>
          </div>
          ...
          <div class="col-md-5 p-1">
            ...
            <div class="small">
              <OptionalList
                :comment="artisan.orderTypesComment"
                :items="artisan.orderTypes"
                :other-items="artisan.otherOrderTypes"
              />
            </div>
          </div>
          ...
          <div class="col-md-4 p-1">
            ...
            <div class="small">
              <OptionalList
                :comment="artisan.featuresComment"
                :items="artisan.features"
                :other-items="artisan.otherFeatures"
              />
            </div>
          </div>
          ...
          <div class="col-md-6 p-1">
            ...
            <div id="artisanSpecies" class="small pb-2">
              ...
              <template v-if="0 !== artisan.speciesDoesnt.length">
                ...
                {{ commaSeparated(artisan.speciesDoesnt) }}
              </template>

              <UnknownValue
                v-if="
                  0 === artisan.speciesDoes.length &&
                  0 === artisan.speciesDoesnt.length
                "
              />
            </div>
          ...
          <div class="col-md-12 p-1">
            ...
            <p class="small mb-1">
              Please mention "<mark class="user-select-all"
                >I found you on getfursu.it<template
                  v-if="!artisan.completenessGood"
                >
                  (BTW it says your data could use some updates)</template
                ></mark
            ...
            <div id="artisanLinks">
              <CardLink
                :url="
                  'https://bewares.getfursu.it/#search:' + artisan.abSearchJson
                "
                label="Check Artists Beware records"
                icon-class="fas fa-balance-scale"
                add-btn-class="border border-primary"
              />
              <CardLink
                :url="artisan.fursuitReviewUrl"
                label="FursuitReview"
                icon-class="fas fa-balance-scale"
                add-btn-class="border border-primary"
              />
              <CardLink
                :url="artisan.websiteUrl"
                label="Official website"
                icon-class="fas fa-link"
              />
              <CardLink
                v-for="item in artisan.pricesUrls"
                :key="item"
                :url="item"
                label="Prices"
              ...
              <CardLink
                :url="artisan.etsyUrl"
                label="Etsy"
                icon-class="fab fa-etsy"
              />
              <CardLink
                :url="artisan.theDealersDenUrl"
                label="The Dealers Den"
                icon-class="fas fa-shopping-cart"
              />
              <CardLink
                :url="artisan.otherShopUrl"
                label="On-line shop"
                icon-class="fas fa-shopping-cart"
              />
              <CardLink
                :url="artisan.furryAminoUrl"
                label="Furry Amino"
                icon-class="fas fa-paw"
              />
              <CardLink
                :url="artisan.scritchUrl"
                label="Scritch"
                icon-class="fas fa-camera"
              />
              <CardLink
                :url="artisan.furtrackUrl"
                label="Furtrack"
                icon-class="fas fa-camera"
              />
              <CardLink
                :url="artisan.linklistUrl"
                label="List of links"
                icon-class="fas fa-link"
              />
            </div>
          ...
          <div
            v-if="0 === artisan.commissionsUrls.length"
            class="col-md-12 p-1"
          >
            <p>Commissions status is not being tracked.</p>

            <p>
              <a :href="Static.getTrackingPath()" target="_blank">Learn more</a>
            </p>
          </div>
          ...
          <template v-else>
            <div
              class="p-1"
              :class="[artisan.isStatusKnown ? 'col-md-6' : 'col-md-8']"
            >
              <template v-if="artisan.isStatusKnown">
                <p v-if="artisan.csTrackerIssue">
                  <i class="inaccurate fas fa-question-circle" /> Note: the
                  software encountered apparent difficulties while figuring out
                  the status; the information is most probably inaccurate/<wbr />incomplete.
                </p>
              ...
              <template v-else>
                <p>
                  <i class="inaccurate fas fa-question-circle" /> Failed to
                  automatically determine commissions status. It should be
                  tracked and updated automatically based on the contents of:
                  <template v-for="item in artisan.commissionsUrls" :key="item">
                    <a :href="item" target="_blank">{{ item }}</a
                    >, <wbr />
                  </template>
                  however the software failed to "understand" the contents. Last
                  time tried on {{ artisan.csLastCheck }} UTC.
                </p>

                <p>
                  <a :href="Static.getTrackingPath()" target="_blank"
                    >Learn more</a
                  >
                </p>
              </template>
            ...
            <div
              class="p-1"
              :class="[
                artisan.isStatusKnown ? 'col-md-6' : 'col-md-4 text-end',
              ]"
            >
              <template v-if="artisan.isStatusKnown">
                <p>
                  Status is tracked and updated automatically based on the
                  contents of:
                  <template v-for="item in artisan.commissionsUrls" :key="item">
              ...
              <img
                v-else
                :src="Static.getTrackingFailedImgSrc()"
                class="img-fluid tracking-failed"
                alt=""
              />
            </div>
          </template>
          ...
          <div class="col-md-12 p-1 pt-3">
            ...
            <p class="small">
              This maker/studio has {{ artisan.completeness }}% data
              completeness. {{ getCompletenessText() }}
        ...
        <div class="modal-footer">
          <button
            ...
            data-bs-target="#artisanUpdatesModal"
...
export default class Link extends Vue {
  ...
  private hasPhotos(): boolean {
    return (
      0 !== this.artisan.photoUrls.length &&
      this.artisan.miniatureUrls.length === this.artisan.photoUrls.length
    );
  }
