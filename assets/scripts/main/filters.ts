...
function initFilters(): void {
    ...
    filters.push(new SpeciesFilterVis('species', 'speciesDoesFilters', 'speciesDoesntFilters'));

    let dataFilters = filters.map(value => value.getFilter());
    dataFilters.push(new AgeAndSfwFilter(getAgeAndSfwConfig()));

    // TODO
    // let filterDtPlugin = new DataTablesFilterPlugin(dataFilters, AgeAndSfwConfig.getInstance());
...
export function setRefreshCallback(refreshCallback: (string) => void): void {
    refreshList = refreshCallback;
}

export function applyFilters(): void {
    refreshFiltersShowButton();
    refreshList($('#filters').serialize());
